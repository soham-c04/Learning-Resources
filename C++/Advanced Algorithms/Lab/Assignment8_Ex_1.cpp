...
vector<int> get_path(int s, int t, int delta){
	vector<int> path;
	int n = graph.size();
	int par[n];
...
int augment(int s, int t, int delta){
	vector<int> path = get_path(s,t,delta);
...
ll scaling_maxflow(int s, int t){
	ll flow = 0,delta = 0;
	for(int w:capacity[0]) delta+=w;
	delta = 1<<(31-__builtin_clz(delta));
	while(delta>=1){
		int bottleneck;
		while(bottleneck = augment(s,t,delta)){
			flow += bottleneck;
			printResidualGraph();
		//		printEdges();
		}
...
int main(){
	...
	cin>>s>>t;
	s--; t--;
	int max_flow = scaling_maxflow(s,t);
